...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    card_user_id : 여신협회 가입 id
    card_user_pw : 여신협회 가입 pw
    
    bank_user_id : 은행 간편조회 가입 id
    bank_user_pw : 은행 간편조회 가입 pw
    ...
    card_user_id = models.CharField(max_length=20, verbose_name="카드아이디", default="", blank=True)
    card_user_pw = models.CharField(max_length=255, verbose_name="카드비밀번호", default="", blank=True)
    
    bank_user_id = models.CharField(max_length=20, verbose_name="뱅크아이디", default="", blank=True)
    bank_user_pw = models.CharField(max_length=255, verbose_name="뱅크비밀번호", default="", blank=True)
    bank_kind = models.CharField(max_length=5, verbose_name="은행명", default="", blank=True)
    bank_number = models.CharField(max_length=20, verbose_name="통장번호", default="", blank=True)
    bank_pw = models.CharField(max_length=20, verbose_name="통장비밀번호", default="", blank=True)
    
    user_biznum = models.CharField(max_length=10, verbose_name="사업자번호", default="", blank=True)
    user_ComName = models.CharField(max_length=100, verbose_name="사업자명", default="", blank=True)
    user_address = models.CharField(max_length=200, verbose_name="사업자주소", default="", blank=True)
...
class AuthSMS(TimeStampedModel):
    ...
    def send_sms(self):
        ...
        body = {
            ...
            "from" : "01052802707",
            "subject" : "정산유",
            "content" : "[정산유] 인증 번호 [{}]를 입력해주세요.".format(self.auth),
            "messages" : [{"to" : self.hp}]
        }
